...
on other properties of the same element.

### Hexadecimal (#RRGGBB[AA])

Next we have Hexadecimal Colors. The are a combination of a hash (`#`) and three or six characters.
...
Recently the hexadecimal notation is extend with an alpha channel, allowing 8 instead of 6 characters, where the last 2
define the alpha of the color. It's shorthand look like `#f00a`, resulting in `#ff0000aa`;
...
Like with the colors there are number of different numeric values.
...
Percentage values are accepted for most properties that use numeric values. It is usefull for creating layouts where the
element should respond on the size of the parent like in responsive webdesign.
...
CSS also provides a way to set variables and re-use/re-assign them on the fly. We will talk about them some more in a
later topic.
...
CSS, just like other programming languagesz has functions. Some of them were already used for the colors such as
`rgb()` and `hsl()` but others are available.
